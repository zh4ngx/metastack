...
- **v0.3 candidate — OpenCode serve injection backend**: Promote the ad-hoc NixOS status-update recipe into a MetaStack-owned OpenCode serve backend. Current recipe: `GET /session`, select the newest session whose `directory` matches the target project root, then `POST /session/<id>/prompt_async` with `{parts:[{type:"text",text:"..."}]}` and verify the message tail. Scope this as a provider-specific injection path for existing OpenCode serve sessions, not as general zellij pane spawning. Once this lands, the NixOS agent can drop its local curl/jq recipe and call the MetaStack primitive instead.
