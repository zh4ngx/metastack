...
- **v0.3 routing core follow-up**: The branch prototype now proves the internal routing envelope, typed target handles, backend capabilities, OpenCode HTTP send, Codex WebSocket send, and route-event vocabulary as a parallel path before DAG integration. Remaining work: target-scoped Codex connection managers, parent/caller reply routing via `reply_to`/`correlation_id`, lifecycle-owned `spawn(agent_spec)`, DAG task integration, and Claude/Huddle plus explicit lossy terminal fallback implementations once addressing semantics are precise.
